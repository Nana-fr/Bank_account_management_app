...
function get_accounts() {
    ...
    echo "<article class='card col-11 col-sm-7 col-md-5 col-xl-4 mx-3 mx-lg-4 mx-xl-5 mb-5 mt-lg-5 p-0'>
        <h5 class='card-header bg-Kobi text-white text-center'>" . $accounts[$i]["name"] . " " . $accounts[$i]["number"] . "</h5>
        <div class='card-body px-0 pb-0'>
        <h5 class='card-title text-center fw-bold mb-3'>Owner: " . $accounts[$i]["owner"] . "</h5>
        <p class='card-text'>Balance:  <span class='fw-bold'>" . $accounts[$i]["amount"] . "</span>€</p>
        <p class='card-text'>Last transaction:  <span class='lastTransaction text-danger fw-bold'>" . $accounts[$i]["last_operation"] . "</span></p>
        <ul class='px-0 pt-4 d-flex justify-content-around btnsBloc'>
            ...
            <li>
            <a href='#' name='$i' type='deposit' class='btn btn-transaction rounded text-success' onClick='deployedForm(this.name, this.type)'>
            <i class='fas fa-coins'></i><i class='fas fa-plus fa-xs ps-1'></i>
            <span class='d-none d-lg-block'>Deposit</span></a>
            </li>
            <li>
            <a href='#' name='$i' type='withdrawal' class='btn btn-transaction rounded text-danger' onClick='deployedForm(this.name, this.type)'>
            <i class='fas fa-coins'></i><i class='fas fa-minus fa-xs ps-1'></i>
            <span class='d-none d-lg-block'>Withdrawal</span></a>
            </li>
            <li>
            <button id='$i' class='btn btn-transaction rounded' onClick='deleteAccount(this.id)'>
